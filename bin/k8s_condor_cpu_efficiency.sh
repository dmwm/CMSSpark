...
for type in "${CMS_TYPES[@]}"; do
    ...
    find "${CPU_EFF_DIR}/${SUB_FOLDER}" -type f \( -name '*.html' -o -name '*.png' \) -mtime +60 -delete
...
for type in "${CMS_TYPES[@]}"; do
    ...
    find "${CPU_EFF_OUTLIER_DIR}/${SUB_FOLDER}" -type f \( -name '*.html' -o -name '*.png' \) -mtime +60 -delete
