...
##H
##H Usage: cron4crab_unique_users.sh <ARGS>
##H Example :
##H    cron4crab_unique_users.sh --keytab ./keytab --output <DIR> --p1 32000 --p2 32001 --host $MY_NODE_NAME --wdir $WDIR
##H Arguments:
...
# run spark function
function run_spark() {
    spark-submit "${spark_submit_args[@]}" "$script_dir/../src/python/CMSSpark/condor_crab_unique_users.py" "$@"
}
