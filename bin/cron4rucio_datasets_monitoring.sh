...
ES_INDEX="test-rucio-datasets-monitoring"
...
function run_spark() {
    ...
    # Run
    spark-submit "${spark_submit_args[@]}" "${spark_py_file}" \
        "${py_input_args[@]}" 2>&1
    # ------------------------------------------------------------------------------------ POST OPS
...
run_spark "${script_dir}/../src/python/CMSSpark/rucio_datasets_monitoring.py" "$hdfs_out" 2>&1
