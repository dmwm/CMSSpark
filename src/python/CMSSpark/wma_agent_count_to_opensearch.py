...
def send(part, opensearch_host, es_secret_file, es_index_template):
    ...
    # No date format for the index: index_mod=""
    idx = client.get_or_create_index(timestamp=time.time(), index_template=es_index_template, index_mod="")
    client.send_opensearch(idx, part, metadata=None, batch_size=10000, drop_nulls=False)
...
@click.option('--es_index', required=True, default=None, type=str,
              help='OpenSearch index template (prefix), i.e.: "test-wma"')
