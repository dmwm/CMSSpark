        ...
        while foundIt:
            ...
            for i in range(n):
                ...
                for j in range(n):
                    ...
                    if gid1!=-1 and gid2!=-1: continue
                    key1=k+(gid1,)
                    key2=k+(gid2,)
                
            #maybe these are related.. look for neighboring dates
                    dMin1=datetime.datetime.strptime(str(rdict[key1][minDateCol]),"%Y%m%d").date()
                    dMax2=datetime.datetime.strptime(str(rdict[key2][maxDateCol]),"%Y%m%d").date()
                    ...
                    if ( (dMin1-dMax2).days == 1 ):
                        ...
                        max_size=maxSize1
                        if maxSize2>maxSize1: max_size=maxSize2
